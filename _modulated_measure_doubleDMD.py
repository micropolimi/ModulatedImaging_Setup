...
class ModulatedMeasure(Measurement):
    ...
    def setup(self):
        ...
        self.settings.New('selected_DMD', dtype=str, choices=('Vialux', 'TexasInstruments'), initial='Vialux', ro=False)
        ...
        self.pattern_gen_vialux = self.app.hardware['VialuxDmdHW']
        self.pattern_gen_texas = self.app.hardware['TexasInstrumentsDmdHW']
    ...
    def run(self):
        ...
        selected_DMD = self.settings['selected_DMD']
        
        if self.settings['DMD_trigger']:
            self.read_from_DMD(selected_DMD)
            print('Read from DMD')
        ...
        if self.camera.settings['acquisition_mode'] == "fixed_length":
            ...
            if self.settings['DMD_trigger']:
                self.start_DMD_projection(selected_DMD)
        ...
        self.camera.hamamatsu.stopAcquisition()
        if self.settings['DMD_trigger']:
                self.end_DMD_projection(selected_DMD)
    ...
    def read_from_DMD(self, DMD):
        
        if DMD == 'Vialux':
            self.pattern_gen_vialux.settings['first_frame'] = self.settings.DMD_first_frame.val
            self.pattern_gen_vialux.settings['last_frame'] = self.settings.DMD_last_frame.val
            
            texp = self.settings.t_acquisition.val
            self.pattern_gen_vialux.settings['time_on'] = texp*1000-10 #50
            self.pattern_gen_vialux.settings['picture_period'] = texp*1000 + 20 #50
            print('time_on', self.pattern_gen_vialux.settings['time_on'])
            print('picture_period', self.pattern_gen_vialux.settings['picture_period'])
            #self.pattern_gen_vialux.read_from_hardware()
            
            nimages = self.pattern_gen_vialux.settings.last_frame.val - self.pattern_gen_vialux.settings.first_frame.val + 1
            ...
            self.set_trigger_from_DMD(DMD)
            self.camera.read_from_hardware()
        
        elif DMD == 'TexasInstruments':
            self.pattern_gen_texas.settings['select_frames'] = True
            self.pattern_gen_texas.settings['first_frame'] = self.settings.DMD_first_frame.val
            self.pattern_gen_texas.settings['last_frame'] = self.settings.DMD_last_frame.val
            self.pattern_gen_texas.load_sequence()
            
            texp = self.settings.t_acquisition.val
            self.pattern_gen_texas.settings['exposure'] = texp*1000000-10000
            self.pattern_gen_texas.settings['dark_time'] = 20000
            picture_period = self.pattern_gen_texas.exposure.val + self.pattern_gen_texas.dark_time.val
            print('time_on', self.pattern_gen_texas.settings['exposure'])
            print('picture_period', picture_period)
            #self.pattern_gen_texas.read_from_hardware()
            
            nimages = self.settings.DMD_last_frame.val - self.settings.DMD_first_frame.val + 1
            self.camera.hamamatsu.setAcquisition('fixed_length')
            self.camera.hamamatsu.setNumberImages(nimages)
            self.camera.hamamatsu.setExposure(texp)
            self.set_trigger_from_DMD(DMD)
            ...
            print("Wrong DMD selection:", DMD)

            
    def set_trigger_from_DMD(self, DMD):
        """
        set the camera trigger
        """
        if DMD == 'Vialux':
            self.pattern_gen_vialux.settings['projection_mode'] = 'master'
        elif DMD == 'TexasInstruments':
            self.pattern_gen_texas.settings['trigger_input'] = False
            self.pattern_gen_texas.settings['trigger_output'] = True #TODO test these settings
        else:
            print("Wrong DMD selection:", DMD)
    ...
    def import_DMD_sequence(self):
        selected_DMD = self.settings.selected_DMD.val
        if selected_DMD == 'Vialux':
            self.pattern_gen_vialux.import_sequence()
        elif selected_DMD == 'TexasInstruments':
            self.pattern_gen_texas.file_browser()
            self.pattern_gen_texas.load_sequence()
        else:
            print("Wrong DMD selection:", selected_DMD)
                                                   
        
    def start_DMD_projection(self, DMD):
        if DMD == 'Vialux':
            # self.pattern_gen_vialux.settings['projection_mode'] = 'master'
            self.pattern_gen_vialux.run_sequence()
            print("Start DMD projection")
        elif DMD == 'TexasInstruments':
            self.pattern_gen_texas.start_sequence()
        else:
            print("Wrong DMD selection:", DMD)

    def end_DMD_projection(self, DMD):
        if DMD == 'Vialux':
            self.pattern_gen_vialux.stop()
            print("End Projection")
        elif DMD == 'TexasInstruments':
            self.pattern_gen_texas.stop_sequence()
        else:
            print("Wrong DMD selection:", DMD)
